  ...
  return (
    <div className={`bg-white rounded-2xl p-5 border ${c.border} shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1`}>
      <div className="flex items-start justify-between mb-4">
        <div className={`w-12 h-12 ${c.light} rounded-xl flex items-center justify-center text-2xl`}>{icon}</div>
        {trend && (
...
function AuditRow({ audit, index }) {
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-4 hover:bg-slate-50 transition-colors ${index > 0 ? 'border-t border-slate-50' : ''}`}>
      <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-sm font-black text-slate-500">{index + 1}</div>
      ...
      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${type.color} hidden md:inline-flex items-center gap-1`}>{type.icon} {type.label}</span>
...
export default function Dashboard() {
  ...
  // Fix: compute random positions once
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50">
      <nav className="bg-slate-900 text-white sticky top-0 z-30 shadow-2xl">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 bg-white/5 rounded-xl px-4 py-2 border border-white/10">
            ...
            <span className="text-sm font-mono font-bold tabular-nums">{time.toLocaleTimeString('ar-SA')}</span>
            <span className="text-slate-400 text-xs">{time.toLocaleDateString('ar-SA', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-sm font-black text-amber-400">{stats.audits > 0 ? Math.round((stats.completed / stats.audits) * 100) : 0}%</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <StatCard icon="📋" label="التدقيقات الكلية"        value={stats.audits}   pct={stats.audits > 0 ? Math.round((stats.completed / stats.audits) * 100) : 0} color="teal" />
          ...
          <StatCard icon="📊" label="متوسط التقييم"           value={`${stats.avgScore}%`} pct={stats.avgScore}                          color="rose" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="md:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            {loading ? (
              <div className="p-8 text-center text-slate-400"><div className="text-3xl mb-2 animate-spin">⏳</div>جارٍ التحميل...</div>
            ) : recentAudits.length === 0 ? (
              <div className="p-8 text-center"><div className="text-4xl mb-2">📭</div><div className="text-slate-400 text-sm">لا توجد تدقيقات بعد</div></div>
            ) : (
              ...
              <div className="space-y-2">
                {[
                  ...
                  <button key={a.label}
                    ...
                    <span>{a.icon}</span><span>{a.label}</span><span className="mr-auto opacity-70">←</span>
                  </button>
                ))}
              </div>
            </div>
